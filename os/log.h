...
#define errorf(fmt, ...)                                                                                                 \
    do {                                                                                                                 \
        printf("\x1b[%dm[%s %d,%d] %s: " fmt "\x1b[0m\n", RED, "ERROR", cpuid(), __safe_pid(), __func__, ##__VA_ARGS__); \
    } while (0)
#else
...
#define warnf(fmt, ...)                                                                                                     \
    do {                                                                                                                    \
        printf("\x1b[%dm[%s %d,%d] %s: " fmt "\x1b[0m\n", YELLOW, "WARN ", cpuid(), __safe_pid(), __func__, ##__VA_ARGS__); \
    } while (0)
#else
...
#define infof(fmt, ...)                                                                                                   \
    do {                                                                                                                  \
        printf("\x1b[%dm[%s %d,%d] %s: " fmt "\x1b[0m\n", BLUE, "INFO ", cpuid(), __safe_pid(), __func__, ##__VA_ARGS__); \
    } while (0)
#else
...
#define debugf(fmt, ...)                                                                                                   \
    do {                                                                                                                   \
        printf("\x1b[%dm[%s %d,%d] %s: " fmt "\x1b[0m\n", GREEN, "DEBUG", cpuid(), __safe_pid(), __func__, ##__VA_ARGS__); \
    } while (0)
#else
...
#define logf(color, text, fmt, ...)                                                                                     \
    do {                                                                                                                \
        printf("\x1b[%dm[%s %d,%d] %s: " fmt "\x1b[0m\n", color, text, cpuid(), __safe_pid(), __func__, ##__VA_ARGS__); \
    } while (0)

#define panic(fmt, ...)                                                                                                                \
    do {                                                                                                                               \
        __panic("\x1b[%dm[%s %d,%d] %s:%d: " fmt "\x1b[0m\n", RED, "PANIC", cpuid(), __safe_pid(), __FILE__, __LINE__, ##__VA_ARGS__); \
    } while (0)
...
#define assert_equals(expecteds, actuals, fmt, ...)                                                                                 \
    do {                                                                                                                            \
        ...
        if (_exp != _act) {                                                                                                         \
            panic("expectation failed: %s != %s, expected: %x actuals: %x" fmt, #expecteds, #actuals, _exp, _act, ##__VA_ARGS__); \
        }                                                                                                                           \
    } while (0)

#define assert_eq(expecteds, actuals)                                                                              \
    do {                                                                                                           \
        typeof(expecteds) _exp = (expecteds);                                                                      \
        typeof(actuals) _act   = (actuals);                                                                        \
        if (_exp != _act) {                                                                                        \
            panic("expectation failed: %s != %s, expected: %x actuals: %x", #expecteds, #actuals, _exp, _act); \
        }                                                                                                          \
    } while (0)
